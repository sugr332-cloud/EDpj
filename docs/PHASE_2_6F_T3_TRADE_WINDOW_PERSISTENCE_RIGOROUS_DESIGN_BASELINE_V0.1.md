...
**Version:** 0.1
**Status:** Implemented（`app/backtest/trade_market_persistence.py`に追加、新規15テスト全通過。実データで6ウィンドウ全て実行し、有用性評価まで完了——§5〜§8。2026-09-06、`buy_price`バックフィル実施後に再実行・再確定済み。利益条件維持率は依然INSUFFICIENT——原因はbuy_price欠落からstation間commodity非重複に変化、§6.1参照）
**Date:** 2026-09-06
...
### 5.1 初回（2026-09-06、backfill前、`data/edpj.db`、1784観測、34系列）
...
`undefined_baseline_count = 0`（全ウィンドウ共通、`sell_price<=0`の行は無かった）。`profit_window_stats`は**全ウィンドウでINSUFFICIENT**（この時点では`buy_price`未収集、2-6F-T1 §10でDeferred済みのバックフィルに起因）。`margin_change_decomposition`も同じ理由で全てINSUFFICIENT。

### 5.2 バックフィル後（2026-09-06、`docs/PHASE_2_6F_T1...md` §11で実施、`data/edpj.db`、2496観測、同じ34系列）

バックフィルにより同じ34ターゲットに対してより広い範囲の観測が再取得され、観測数が1784→2496へ増加した（`buy_price`/`supply`/`received_at`も全行で判明済み）。全ウィンドウで再実行した結果:

```text
window   eligible  comparison(率)      unchanged_rate  decrease_rate  material_at_window  median_gap  censored(率)         profit_status
 5分       2496     334  (13.4%)          100.00%          0.00%           0.00%              0:02:26   2162 (86.6%)   INSUFFICIENT
10分       2496     634  (25.4%)          100.00%          0.16%           0.00%              0:05:23   1862 (74.6%)   INSUFFICIENT
15分       2496     878  (35.2%)           99.43%          3.53%           0.57%              0:06:31   1618 (64.8%)   INSUFFICIENT
30分       2496    1178  (47.2%)           99.58%          4.33%           0.42%              0:11:20   1318 (52.8%)   INSUFFICIENT
60分       2496    1388  (55.6%)           98.20%          6.84%           1.51%              0:28:12   1108 (44.4%)   INSUFFICIENT
120分      2496    1688  (67.6%)           98.05%          6.93%           1.72%              0:57:05    808 (32.4%)   INSUFFICIENT
```

**価格系の数値は§5.1とほぼ同じ傾向（60〜120分が最も比較可能率が高く、5〜15分はcensoring率が高い）を維持している**——観測数が増えても結論は変わらない。しかし**`profit_status`は依然として全ウィンドウでINSUFFICIENT**——今度は`buy_price`が無いからではない（§6参照、原因が変わった）。
...
### 6.1 バックフィル後に判明した新しい事実（`profit_window_stats`は依然INSUFFICIENT、ただし理由が変わった）

`docs/PHASE_2_6F_T1_...md` §11で`buy_price`/`supply`/`received_at`のバックフィルを実施した後も、本書の`compute_profit_window_stats`/`compute_margin_change_decomposition`は全ウィンドウで`eligible_count=0`のまま変わらなかった。原因は`buy_price`の欠落ではなく、**現在実データとして存在する2 stationがcommodityの共通集合を持たない**（station 3221821952の30品目とstation 3789719552の4品目に重複が無い）ため、Buy→輸送→SellというTradeルート自体が1つも構成できないことにある。これは§7-D（station依存性）で既に指摘していた「比較可能サンプルの98%以上が単一station由来」という制約の、**より強い形**（利益条件については単一stationでは原理的に定義不能）であり、本書の結論に新しい制約として追記する（§8）。
...
- 「現在観測された利益：+50 Cr/t」に対して、**利益条件維持率は全ウィンドウでINSUFFICIENT**——バックフィル後の再検証（§6.1）で判明した通り、これはもはや`buy_price`の欠落ではなく、**現在の2 stationがcommodityを共有していないため、Buy→Sellの経路自体が1つも組めない**ことによる。バックフィルを追加しても解消しない種類の制約であり、Trade固有の「利益条件がどれだけ続くか」という最も欲しい情報は、station多様性が改善するまで一切提供できない。
- 価格維持率（sell側のみ）は60〜120分については「意味のある形」で表示できる: 例えば「過去観測では、この商品はT0から120分後までに約1.7%の確率で5%以上のセル価格下落が観測された（station 3221821952のみ、観測gap中央値57分、比較可能1688件中2496件）」という言い方は、実データに基づいた具体的な文章になる。
...
## 8. 最終結論（バックフィル後、`data/edpj.db`、2496観測で確定）

| 経過時間 | 有効T0 | 比較可能 | 価格維持率 | 下落率 | material decrease率 | 利益条件維持率 | median gap | censoring |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 5分 | 2496 | 334 (13.4%) | 100.00% | 0.00% | 0.00% | INSUFFICIENT* | 0:02:26 | 2162 (86.6%) |
| 10分 | 2496 | 634 (25.4%) | 100.00% | 0.16% | 0.00% | INSUFFICIENT* | 0:05:23 | 1862 (74.6%) |
| 15分 | 2496 | 878 (35.2%) | 99.43% | 3.53% | 0.57% | INSUFFICIENT* | 0:06:31 | 1618 (64.8%) |
| 30分 | 2496 | 1178 (47.2%) | 99.58% | 4.33% | 0.42% | INSUFFICIENT* | 0:11:20 | 1318 (52.8%) |
| 60分 | 2496 | 1388 (55.6%) | 98.20% | 6.84% | 1.51% | INSUFFICIENT* | 0:28:12 | 1108 (44.4%) |
| 120分 | 2496 | 1688 (67.6%) | 98.05% | 6.93% | 1.72% | INSUFFICIENT* | 0:57:05 | 808 (32.4%) |

*利益条件維持率のINSUFFICIENTは、`docs/PHASE_2_6F_T1_...md` §11のバックフィル実施後も解消していない——原因が`buy_price`の欠落から「2 stationにcommodityの共通集合が無い」という別の構造的制約に変わった（§6.1）。

1. **このデータで何分後まで信頼性を定量化できるか**: 実用的な下限は概ね**60分**。5〜30分は比較可能率が13.4〜47.2%しかなく、censoring率が過半数（30分でも52.8%）を超えるため、統計として成立していても「実際の判断に足る母数」とは言えない。60〜120分は比較可能率が55.6〜67.6%まで上がり、初めて「定量化できた」と言える水準に達する（バックフィルで観測数が1784→2496に増えても、この傾向自体は変わらなかった）。
2. **どの時間窓がTrade判断に最も有用か**: **60分・120分**。ただしobservation gap自体がウィンドウの約半分を占めるため、「正確に60分後」ではなく「30分〜90分の間のどこか」を測っていることに注意が必要。
3. **価格維持率と利益条件維持率のどちらが有用か**: 現状は**価格維持率のみ**が計算可能。利益条件維持率はTradeにとって本来最も重要な指標だが、バックフィル実施後も全ウィンドウでINSUFFICIENTのまま——**原因が「`buy_price`未収集」から「2 stationにcommodityの共通集合が無い」という、バックフィルでは解消できない別の構造的制約に変わった**（§6.1）。
4. **station/commodity偏りによる限界**: 比較可能サンプルの98%以上が単一station由来。**現在の結果はこの1 stationにしか一般化できない。** さらに利益条件については、2つ目のstationとcommodityが1つも重複しないため、原理的に「Buy→Sell」のTradeルート自体を1つも構成できない——station依存性の中でも特に深刻な形。
5. **実際のTrade推薦UIに表示するなら何を表示すべきか**: 60〜120分については「過去観測ベースで、この商品はこのstationでtに対しX%の確率で5%以上のセル価格下落が観測された（観測件数・観測gapを併記）」という、station限定・観測事実ベースの文言。5〜30分については「データ不足のため定量化できません」と明示する。利益条件維持率については「現在利用可能な市場データでは、Buy→Sellの経路を構成できるstationの組み合わせが存在しないため評価できません」と明示すべきである。**「到着時に儲かる確率」「N分は安全」という表現は一貫して禁止**（§9の要求通り）。
6. **現状データでは「有用」と判断できるか、それともINSUFFICIENTか**:
   ...
   - 利益条件維持率・非対称変動分解: **INSUFFICIENT**（構造的。バックフィル実施済みだが、2 station間でcommodityの共通集合が無いという別の制約により解消せず。station多様性の改善以外に解決策が無い）。

**「数字が計算できたこと」と「Trade判断に有用であること」は明確に別**——本書の6ウィンドウ全てで数字自体は計算できたが、有用と言えるのは60〜120分の価格維持率のみであり、それも単一stationの証拠という限定付きである。利益条件維持率はバックフィルというコストを払った後も、依然としてTrade判断に一切使えない状態である。
