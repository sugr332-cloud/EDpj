...
### 5.9 診断5: 単変量チェック — 候補特徴量は「ミスケースだけ」では有望に見えたが、対照群（全holdout）比較で否定

`distance_to_arrival`について、過大予測ミス上位4件を目視した際は100倍以上の乖離が確認でき、有望に見えた。しかし「ミスケースだけを見て原因と判断する」誤りを避けるため、**同じ乖離パターンが正解ケースでも同程度に起きているか**を対照群として検証した。

```text
                                  過大予測ミスでの外れ値率    全holdoutケースでの外れ値率
distance_to_arrival(>5倍/<0.2倍)  31.7%（33/104）           34.0%（124/365） ← ミス側の方がむしろ低い
surface_pressure(>5倍/<0.2倍)     11.5%                      7.2%
atmosphere_composition(L1>30pt)   7.9%                       6.1%
```

`distance_to_arrival`は完全に無相関（ミス側がむしろ低い）——目視で見た劇的な例は、母集団全体に存在するばらつきをミスケース側だけ見て原因と錯覚したものだった。`surface_pressure`/`atmosphere_composition`はミス側でわずかに高いが効果量は小さく、単独では決定打にならない。**単変量では候補特徴量に明確な説明力は確認できなかった。**
